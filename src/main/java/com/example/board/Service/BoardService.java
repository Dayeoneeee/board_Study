...
//    private BoardRepository boardRepository;
    ...
    private final ModelMapper modelMapper; //AppConfig에서 선언했기 때문에 = new MpdelMapper() 생략 가능

    //Controller과 Service는 DTO로 전달
    public void insert(BoardDTO boardDTO) {   //삽입, 입력 폼에서 입력받은 내용을 데이터베이스에 저장
        //DTO를 Entity로 반드시 변환 해야 함.
        //map은 변수와 값으로 구성된 배열
        //boardDTO 변수들은 BoardEntity의 변수에 맞게 변환
        BoardEntity boardEntity = modelMapper.map(boardDTO, BoardEntity.class);

        boardRepository.save(boardEntity);
    }
    public void update(BoardDTO boardDTO) {   //수정, 수정폼에서 수정한 내용을 데이터베이스에 저장
    ...
    public List<BoardDTO> list() {     //전체목록, 데이터베이스에서 모든 데이터를 화면에 출력
        //필드:필드:필드-레코드(map)
        //레코드:레코드:레코드(List)
        List<BoardEntity> boardEntities = boardRepository.findAll();    //모두조회
    ...
    public BoardDTO read(Integer id) {     //개별정보, 게시글 번호의 데이터를 화면에 출력
